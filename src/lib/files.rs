...
pub fn get_files (path: &str, folders_only: bool) -> Vec<Template> {
    ...
    for entry in fs::read_dir(path).unwrap() {
        ...
        files.push(Template { name: filename.to_string(), files: Vec::new(), has_folder: false });
        }
        ...
        if !folders_only {
            ...
            if path.is_dir() {
                for entry in fs::read_dir(&path).unwrap() {
                    ...
                    inner_files.push(TemplateFile { name: sub_filename.to_string(), path: sub_path.to_string_lossy().to_string(),  is_folder: sub_path.is_dir() });
            }
        }
            if path.is_file() {
                println!("fuck");
                inner_files.push(TemplateFile { name: filename.to_string(), path:path.to_string_lossy().to_string(), is_folder: path.is_dir() });
            }
            files.push(Template { name: filename.to_string(), files: inner_files, has_folder: path.is_dir() });
        }
    }
    return files;
}
